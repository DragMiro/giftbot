...
_AFK_MODES: dict[str, dict[str, object]] = {
    "default": {
        "emoji": "🌙",
        "label": "AFK",
        "status": "недоступен (AFK)",
        "aliases": ("afk", "default", "оффлайн", "офлайн"),
    },
    "sleep": {
        "emoji": "😴",
        "label": "спит",
        "status": "спит",
        "aliases": ("спит", "сон", "sleep", "сплю", "дремлет"),
    },
    "eat": {
        "emoji": "🍽",
        "label": "кушает",
        "status": "кушает",
        "aliases": ("кушает", "ест", "еда", "eat", "обед", "ужин", "завтрак", "перекус"),
    },
    "toilet": {
        "emoji": "🚽",
        "label": "в туалете",
    ...
    "shower": {
        ...
        "label": "в душе",
        "status": "в душе",
        "aliases": ("душ", "душе", "shower", "мыться", "купается"),
    },
    "work": {
        "emoji": "💼",
        "label": "работает",
        "status": "работает",
        "aliases": ("работа", "работает", "work", "занят", "busy", "дела"),
    },
    "gym": {
        "emoji": "🏋️",
        "label": "на тренировке",
        "status": "на тренировке",
        "aliases": ("тренировка", "зал", "gym", "спорт", "качается", "бегает"),
    },
    "drive": {
        "emoji": "🚗",
        "label": "за рулём",
        "status": "за рулём",
        "aliases": ("руль", "за рулем", "за рулём", "drive", "едет", "в дороге", "машина"),
    },
    "study": {
        "emoji": "📚",
        "label": "учится",
        "status": "учится",
        "aliases": ("учеба", "учёба", "учится", "study", "пары", "лекция", "экзамен"),
    },
    "game": {
        "emoji": "🎮",
        "label": "играет",
        "status": "играет",
        "aliases": ("игра", "играет", "game", "катка", "дотка", "кс"),
    },
    "walk": {
        "emoji": "🚶",
        "label": "гуляет",
        "status": "гуляет",
        "aliases": ("гуляет", "прогулка", "walk", "на улице"),
    },
    "call": {
        "emoji": "📞",
        "label": "на звонке",
        "status": "на звонке",
        "aliases": ("звонок", "звонит", "call", "созвон", "митинг", "созвоне"),
    },
}

_AFK_HEADER = """Ты — ИИ-менеджер пользователя {owner_name}.
Пользователь сейчас {afk_status}. Ты временно отвечаешь за него в личных сообщениях.

Изучи историю переписки ниже — пойми, кто собеседник, какой у вас стиль общения, о чём вы обычно говорите.

Правила ответа:
1. Кратко представься: ты ИИ-менеджер {owner_name}, {owner_name} сейчас {afk_status}, поэтому отвечаешь ты.
...
if loader:
    ...
    class CursorAgentMod(loader.Module):
        ...
        strings = {
            ...
            "afk_on": (
                "🌙 <b>AFK-режим включён</b> — {mode_label}\n\n"
        ...
        def _afk_mode_info(mode_key: str) -> dict[str, object]:
            return _AFK_MODES.get(mode_key, _AFK_MODES["default"])

        @classmethod
        def _resolve_afk_mode(cls, raw: str) -> str | None:
            needle = (raw or "").strip().lower()
            if not needle:
                return None
            if needle in _AFK_MODES:
                return needle
            for key, info in _AFK_MODES.items():
        ...
        def _afk_modes_help(self) -> str:
            ...
            for key, info in _AFK_MODES.items():
                aliases = info.get("aliases", ())
                sample = ", ".join(str(a) for a in aliases[:3])
                lines.append(
                    f"{info['emoji']} <b>{_escape(str(info['label']))}</b> — "
                    f"<code>{_escape(sample)}</code>"
                )
            return self.strings("afk_modes_list").format(modes="\n".join(lines))
        ...
        async def cursorcmd(self, message: Message) -> None:
            ...
            if not prompt:
                await utils.answer(
                    ...
                    "▫️ <code>.afkcursor спит</code> — AFK с режимом\n"
                    "▫️ <code>.afkcursor list</code> — режимы AFK\n\n"
        ...
        async def afkcursorcmd(self, message: Message) -> None:
            ...
            if args in ("list", "режимы", "modes", "help", "?"):
            ...
            if self._afk_enabled:
                ...
                await utils.answer(
                    message,
                    self.strings("afk_already").format(mode_label=self._afk_mode_label()),
                )
                return
            ...
            await utils.answer(
                message,
                self.strings("afk_on").format(mode_label=self._afk_mode_label()),
            )
