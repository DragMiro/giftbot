...
_AFK_INTRO_FIRST = (
    "1. Это твой первый ответ этому собеседнику в этой AFK-сессии — "
    "кратко представься: ты ИИ-менеджер {owner_name}, пользователь сейчас не может ответить, "
    "поэтому отвечаешь ты."
)
_AFK_INTRO_REPEAT = (
    "1. Ты уже отвечал этому собеседнику в этой AFK-сессии — "
    "НЕ представляйся заново и не повторяй приветствие. Отвечай сразу по существу."
)
    ...
    class CursorAgentMod(loader.Module):
        ...
        def _wrap_afk_prompt(
            ...
            *,
            ...
            intro_rule = (
                _AFK_INTRO_FIRST.format(owner_name=owner_name)
                if first_reply
                else _AFK_INTRO_REPEAT
            )
            return (
                _AFK_HEADER.format(
        ...
        async def _ask_afk(self, message: Message, prompt: str) -> None:
            ...
            try:
                ...
                first_reply = message.chat_id not in self._afk_greeted
                ...
                self._afk_greeted.add(message.chat_id)
